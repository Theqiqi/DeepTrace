...
| `Error: cannot read file: <path>` | File missing/unreadable (script/batch/ptrscan/conversion inputs) | Check the path; for scripts, also run `script check` for a syntax diagnosis |
| `Error: script check failed: ...` (exit 2) | The `.aa` script failed syntax/hook-structure/assembly validation | Fix the reported line/symbol/instruction, then re-check (see [User Manual 12](USER_MANUAL.md#12-script-engine-script)) |
| `Error: invalid ptrscan config: ...` / `invalid batch config: ...` (exit 2) | JSON config failed validation before execution | Check the field names/types against [User Manual 2.6](USER_MANUAL.md#26-batch-read--write--mem-batch-readwrite-filejson---format-tablecsvjson---out-file) / [7.3](USER_MANUAL.md#73-pointer-chain-scan--resolve-ptrscan-filejson); the target is untouched |
| `Error: NotFound` (ptrscan) | The anchor `module` in the config isn't loaded | Use a loaded module name (see `module list`) or remove `module` |
| `Error: Timeout` (shellcode free) | The shellcode's thread is still running | Wait and retry; `free` deliberately refuses to free running code |
...
- **Scripts are for 64-bit code**: script assembly targets x64; `alloc`'d memory is allocated with full access (read/write/execute) by design.
- **`script_alloc_near` can fail**: if a script's `alloc(name,size,"module.dll"+off)` finds no free region within ±2 GB of the anchor it errors instead of silently placing elsewhere — move the anchor or drop the third argument.
- **Pointer scans are heuristic**: a snapshot may include false positives; always use the rescan step after a restart (see [FAQ 16](FAQ.md#16-resolve-ptrscan-finds-nothing--or-finds-chains-that-stop-working-after-a-restart)).
- **`hex2bin` wants compact hex**: no spaces inside the hex argument (use `bin2hex` output only after removing spaces, or paste compact hex).
...
```
deeptrace_cli -v        :: should show deeptrace_cli v2.13.0
deeptrace_cli -h        :: should show the command list (incl. script / batch / ptrscan / bin2hex)
