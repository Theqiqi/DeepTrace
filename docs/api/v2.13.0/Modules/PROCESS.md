...
| Parameter | Type | Description |
|-----------|------|-------------|
| `out_mask` | `uint32_t*` | output parameter, the actual PROCESS_* access mask granted by the last successful `attach` |

### Return Value

| Return value | Meaning |
|--------------|---------|
| `Result::Ok` | query succeeded; `out_mask` holds the granted access mask |
| `Result::InvalidArg` | `out_mask == nullptr` |
| `Result::NotAttached` | no session exists (`session.pid == 0`) |

### Description

Returns the **actual** process access mask the last successful `attach` was granted, with Windows `PROCESS_*` bit semantics (e.g. `PROCESS_VM_READ`, `PROCESS_VM_WRITE`, `PROCESS_VM_OPERATION`, `PROCESS_CREATE_THREAD`, `PROCESS_QUERY_INFORMATION`). A full-access attach records `PROCESS_ALL_ACCESS`; a degraded attach (e.g. opened with a fallback right set after the full-access attempt failed) records the degraded mask. The mask is recorded during `attach` itself — **no extra system probes are made**. Typical use: after an `attach` that may have been silently downgraded, check which operations are actually available and surface a clear message to the user instead of letting later calls fail obscurely. `detach` resets the recorded mask to 0.

Prerequisites: `attach(pid)` succeeded (otherwise `NotAttached`). Postconditions: none.

### Example

```cpp
uint32_t mask = 0;
if (deeptrace::session_permissions(&mask) == deeptrace::Result::Ok) {
    if ((mask & PROCESS_VM_READ) == 0) {
        std::cerr << "warning: no VM_READ right, memory reads may fail\n";
    }
}
```

> `PROCESS_VM_READ` etc. are Windows SDK macros; include `<windows.h>` in the caller for the constants, or compare against the numeric values.
