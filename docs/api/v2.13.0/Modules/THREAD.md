...
| Parameter | Type | Description |
|-----------|------|-------------|
| `addr` | `uintptr_t` | entry address of the new remote thread inside the target process; must not be 0 |
| `out_tid` | `uint32_t*` | output parameter, the created thread's ID (may be `nullptr` to ignore) |
...
Creates a remote thread in the target process whose entry point is an **arbitrary address** `addr`. Unlike the injection APIs, this function does not write any code and does not manage a record — it is the low-level primitive used to "trigger" code already present in the target (e.g. a `script_alloc`/`shellcode_alloc` buffer that was previously written, or an existing function). The thread starts immediately at `addr`; the call does not wait for it to finish. Typical use: run an injected assembly routine on demand, repeatedly if desired (each call spawns a new thread).

Prerequisites: `attach(pid)` done; `addr` must point to executable code in the target. Postconditions: a new remote thread is running.

### Example

```cpp
uint32_t tid = 0;
// addr was returned by script_alloc / shellcode_alloc earlier
if (deeptrace::thread_create_at(addr, &tid) == deeptrace::Result::Ok) {
    std::cout << "thread started, tid=" << tid << "\n";
}
```
...
- [deeptrace::shellcode_alloc](INJECT.md#deeptraceshellcode_alloc)
