...
ParseResult parse1(const char* a1) { return parse({"pmem_cli", a1}); }
...
TEST(Parser, HelpShort) {
    auto r = parse({"pmem_cli", "-h"});
    EXPECT_TRUE(r.ok);
    EXPECT_TRUE(r.req.help);
}

TEST(Parser, HelpLong) {
    auto r = parse({"pmem_cli", "--help"});
    EXPECT_TRUE(r.ok);
    EXPECT_TRUE(r.req.help);
}

TEST(Parser, Version) {
    auto r = parse({"pmem_cli", "-v"});
    EXPECT_TRUE(r.ok);
...
TEST(Parser, PidShort) {
    auto r = parse({"pmem_cli", "-p", "1234", "ps", "list"});
    ASSERT_TRUE(r.ok);
...
TEST(Parser, PidLong) {
    auto r = parse({"pmem_cli", "--pid", "999", "ps", "list"});
    ASSERT_TRUE(r.ok);
    EXPECT_TRUE(r.req.pid_set);
...
TEST(Parser, PidAfterCommand) {
    auto r = parse({"pmem_cli", "ps", "list", "-p", "42"});
    ASSERT_TRUE(r.ok);
    EXPECT_EQ(r.req.pid, 42u);
}

TEST(Parser, PidMissingValue) {
    auto r = parse({"pmem_cli", "-p"});
    EXPECT_FALSE(r.ok);
    EXPECT_EQ(r.exit_code, 2);
}

TEST(Parser, PidInvalid) {
    auto r = parse({"pmem_cli", "-p", "0xZZ", "ps", "list"});
    EXPECT_FALSE(r.ok);
    EXPECT_EQ(r.exit_code, 2);
...
TEST(Parser, UnknownOption) {
    auto r = parse({"pmem_cli", "-x", "ps", "list"});
    EXPECT_FALSE(r.ok);
    EXPECT_EQ(r.exit_code, 2);
}
...
TEST(Parser, UnknownGroup) {
    auto r = parse({"pmem_cli", "bogus", "list"});
    EXPECT_FALSE(r.ok);
    EXPECT_EQ(r.exit_code, 2);
...
TEST(Parser, UnknownSubcommand) {
    auto r = parse({"pmem_cli", "ps", "frobnicate"});
    EXPECT_FALSE(r.ok);
    EXPECT_EQ(r.exit_code, 2);
...
TEST(Parser, MissingRequiredArg) {
    auto r = parse({"pmem_cli", "mem", "read"});
    EXPECT_FALSE(r.ok);
    EXPECT_EQ(r.exit_code, 2);
...
TEST(Parser, InvalidAddress) {
    auto r = parse({"pmem_cli", "mem", "read", "not_an_addr"});
    EXPECT_FALSE(r.ok);
    EXPECT_EQ(r.exit_code, 2);
...
TEST(Parser, AddressHexAndDec) {
    auto r = parse({"pmem_cli", "mem", "read", "0x140001000"});
    ASSERT_TRUE(r.ok);
...
TEST(Parser, MemReadDefaults) {
    auto r = parse({"pmem_cli", "mem", "read", "5368709120"});
    ASSERT_TRUE(r.ok);
...
TEST(Parser, MemReadExplicitArgs) {
    auto r = parse({"pmem_cli", "mem", "read", "0x1000", "16", "dec"});
    ASSERT_TRUE(r.ok);
...
TEST(Parser, MemReadBadFormat) {
    auto r = parse({"pmem_cli", "mem", "read", "0x1000", "4", "octal"});
    EXPECT_FALSE(r.ok);
    EXPECT_EQ(r.exit_code, 2);
}

TEST(Parser, MemWriteHexBytes) {
    auto r = parse({"pmem_cli", "mem", "write", "0x1000", "DEADBEEF"});
    ASSERT_TRUE(r.ok);
...
TEST(Parser, MemWriteOddHex) {
    auto r = parse({"pmem_cli", "mem", "write", "0x1000", "ABC"});
    EXPECT_FALSE(r.ok);
    EXPECT_EQ(r.exit_code, 2);
}

TEST(Parser, MemWriteBadChar) {
    auto r = parse({"pmem_cli", "mem", "write", "0x1000", "DEADGG"});
    EXPECT_FALSE(r.ok);
    EXPECT_EQ(r.exit_code, 2);
}

TEST(Parser, MemReadvalType) {
    auto r = parse({"pmem_cli", "mem", "readval", "0x1000", "qword"});
    ASSERT_TRUE(r.ok);
    EXPECT_EQ(r.req.args[1], "qword");
    auto bad = parse({"pmem_cli", "mem", "readval", "0x1000", "int"});
    EXPECT_FALSE(bad.ok);
}

TEST(Parser, PsAttachPid) {
    auto r = parse({"pmem_cli", "ps", "attach", "1234"});
    ASSERT_TRUE(r.ok);
    EXPECT_EQ(r.req.args.size(), 1u);
    EXPECT_EQ(r.req.args[0], "1234");
    auto bad = parse({"pmem_cli", "ps", "attach", "0"});
    EXPECT_FALSE(bad.ok);
    auto bad2 = parse({"pmem_cli", "ps", "attach", "1.5"});
    EXPECT_FALSE(bad2.ok);
}

TEST(Parser, PsKillDefaultExitCode) {
    auto r = parse({"pmem_cli", "ps", "kill"});
    ASSERT_TRUE(r.ok);
    EXPECT_EQ(r.req.args.size(), 1u);
...
TEST(Parser, DebugStepDefaultTid) {
    auto r = parse({"pmem_cli", "debug", "step"});
    ASSERT_TRUE(r.ok);
    EXPECT_EQ(r.req.args.size(), 1u);
    EXPECT_EQ(r.req.args[0], "0");
    auto r2 = parse({"pmem_cli", "debug", "step", "77"});
    ASSERT_TRUE(r2.ok);
    EXPECT_EQ(r2.req.args[0], "77");
}

TEST(Parser, DebugHbreakDefaults) {
    auto r = parse({"pmem_cli", "debug", "hbreak", "0x1000"});
    ASSERT_TRUE(r.ok);
...
TEST(Parser, HwTypeValid) {
    auto r = parse({"pmem_cli", "debug", "hbreak", "0x1000", "2", "8"});
    ASSERT_TRUE(r.ok);
    auto bad = parse({"pmem_cli", "debug", "hbreak", "0x1000", "9", "1"});
    EXPECT_FALSE(bad.ok);
}

TEST(Parser, DisasmAtDefaultCount) {
    auto r = parse({"pmem_cli", "disasm", "at", "0x1000"});
    ASSERT_TRUE(r.ok);
    EXPECT_EQ(r.req.args[1], "10");
}

TEST(Parser, PatternValid) {
    auto r = parse({"pmem_cli", "resolve", "scan", "48 8B ?? ?? 00"});
    ASSERT_TRUE(r.ok);
    auto bad = parse({"pmem_cli", "resolve", "scan", "48 Z"});
    EXPECT_FALSE(bad.ok);
    auto empty = parse({"pmem_cli", "resolve", "scan", ""});
    EXPECT_FALSE(empty.ok);
}

TEST(Parser, TooManyArguments) {
    auto r = parse({"pmem_cli", "ps", "list", "extra"});
    EXPECT_FALSE(r.ok);
    EXPECT_EQ(r.exit_code, 2);
...
TEST(Parser, AsmAssembleFlags) {
    auto r = parse({"pmem_cli", "asm", "assemble", "mov rax, 1", "--hex"});
    ASSERT_TRUE(r.ok);
    EXPECT_EQ(r.req.args[0], "mov rax, 1");
    EXPECT_EQ(r.req.args[1], "--hex");
    auto r2 = parse({"pmem_cli", "asm", "assemble", "nop", "--c-array"});
    ASSERT_TRUE(r2.ok);
...
TEST(Parser, ShellcodeHexBytes) {
    auto r = parse({"pmem_cli", "shellcode", "inject", "4831C0C3"});
    ASSERT_TRUE(r.ok);
    EXPECT_EQ(r.req.args[0], "4831C0C3");
    auto bad = parse({"pmem_cli", "shellcode", "inject", "4831C0C"});
    EXPECT_FALSE(bad.ok);
}

TEST(Parser, WatchAddTypes) {
    auto r = parse({"pmem_cli", "watch", "add", "mywatch", "0x1000", "float"});
    ASSERT_TRUE(r.ok);
    EXPECT_EQ(r.req.args[2], "float");
    auto bad = parse({"pmem_cli", "watch", "add", "mywatch", "0x1000", "bogus"});
    EXPECT_FALSE(bad.ok);
}

TEST(Parser, ModuleDumpDefaultFile) {
    auto r = parse({"pmem_cli", "module", "dump", "kernel32.dll"});
    ASSERT_TRUE(r.ok);
...
TEST(Parser, HelpTextIsAscii) {
    ...
    EXPECT_NE(help.find("pmem_cli"), std::string::npos);
