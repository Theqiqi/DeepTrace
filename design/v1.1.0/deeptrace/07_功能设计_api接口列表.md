# deeptrace 静态库 - 功能设计(API 接口列表)

> 本列表即 deeptrace 静态库公共 API 完整清单。所有类型使用标准 C++ 类型,
> 不暴露 windows.h 类型,保证 CLI 无需包含平台头文件。

## 1. 公共头文件

```cpp
#include "deeptrace.h"        // 唯一公共头文件,位于 deeptrace/include/
namespace deeptrace { ... }
```
...
| 层 | 目录 | 职责 |
|----|------|------|
| Domain 层 | src/domain/ | 类型定义(types.h) |
| 算法层 | src/algorithm/ | 纯计算:hex、AOB 匹配、格式化(无平台 API);x64 解码/编码归 Infrastructure |
| Infrastructure 层 | src/infrastructure/ | 基础能力:按能力类型分子目录(process/memory/module/thread/debug/inject/disassembly/logging 等),跨平台以文件后缀区分 |
| Service 层 | src/service/ | 公共 API:组装 算法+数据结构+基础设施能力,错误码与状态持久化 |
| utils 层 | src/utils/ | 便捷门面:log.h 等,只转发调用不实现能力(按需) |
