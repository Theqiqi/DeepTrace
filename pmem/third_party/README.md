...
- **构建方式**:非 vcpkg——vcpkg 官方有 keystone port,但默认全架构 LLVM 构建需数十分钟;手动下载后可裁剪 `LLVM_TARGETS_TO_BUILD=X86` 加速(见 `pmem/CMakeLists.txt` 中 `add_subdirectory(third_party/keystone/llvm)` 的集成方式)
- **关键约束**:
...
- **注意**:此 python 仅用于构建,不参与运行时;若未来 Windows 侧安装真实 python,可删除本目录并在 pmem/CMakeLists.txt 改用系统 PYTHON_EXECUTABLE
